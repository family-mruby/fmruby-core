...
static volatile int hid_device_connected = 0;
                                         ...
                                         void *arg)
{
    switch (event) {
        ...
        case HID_HOST_INTERFACE_EVENT_DISCONNECTED:
            FMRB_LOGI(TAG, "HID Device Disconnected");
            hid_device_connected = 0;
            if (hid_device != NULL) {
...
static void hid_host_task(void *arg)
{
    ...
    while (usb_running) {
        ...
        // If device connected, we could read reports here
        if (hid_device_connected && hid_device != NULL) {
            // Keyboard/Mouse reports would be handled here
            // For now, just log periodically
            static int count = 0;
            if (++count >= 50) {  // Every 5 seconds
                FMRB_LOGI(TAG, "HID device active");
                count = 0;
            }
        }
    }
...
void usb_hid_conn_check_stop(void)
{
    ...
    // Close HID device if still connected (may already be closed by disconnect callback)
    if (hid_device != NULL) {
        hid_host_device_stop(hid_device);
        hid_host_device_close(hid_device);
        hid_device = NULL;
    }
    hid_device_connected = 0;
