...
static void fmrb_os_main_loop(void)
{
    ...
    while (1) {
        ...
        if (g_gfx_context) {
            ...
            fmrb_gfx_draw_string(g_gfx_context, "LovyanGFX Compatible API Test", 10, 200, FMRB_COLOR_WHITE);

            // Draw frame counter
            char frame_text[64];
            snprintf(frame_text, sizeof(frame_text), "Frame: %d", frame_count);
            fmrb_gfx_draw_string(g_gfx_context, frame_text, 10, 240, FMRB_COLOR_YELLOW);
        ...
        */
        ...
        int delay_needed = frame_delay_ms - frame_time_ms;

        ESP_LOGI(TAG, "Frame %d: took %dms, target %dms, delaying %dms",
                 frame_count, frame_time_ms, frame_delay_ms, delay_needed > 0 ? delay_needed : 0);

        if (delay_needed > 0) {
            fmrb_hal_time_delay_ms(delay_needed);
        }
