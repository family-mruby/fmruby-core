...
- **GfxBlock#draw は「再生」のたびにブロックを再実行して記録し直す**
  ...
  バーの再描画だけで 32 個/回を実機で計測)。GfxBlock の本領は帯域削減
  (WROVER へ差分レジスタだけ送る) で、Retro の SPI リンクでは重要な
  性質。つまり現状は「ワイヤバイトを節約する代わりに再生ごとの割り当て
  を払う」**トレードオフ**であり、どちらを取るかはそのパスで希少な資源
  (Retro の帯域か、mruby プールか) で決める。launcher のスクロールは
  差し替えても送信コマンド数が増えない (frame 分はむしろ減る) ことを
  確認した上で直接描画にした (update_scrollbar_thumb)。
  **本質的な解は GfxBlock#draw の割り当てフリー化** (Recorder と
  コマンド配列の使い回し、レジスタのインプレース更新) で、実現すれば
  このトレードオフ自体が消える。
