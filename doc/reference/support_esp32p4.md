...
- [x] lgfx_tab5: パネル自動判定（ILI9881C / ST7123 / ST7121 をタッチ I2C プローブで切替。NARYAv4量産対応）
...
| 判定条件 | 表示ドライバ | タッチドライバ | DSI lane | DPI | hsync (back/pulse/front) | vsync (back/pulse/front) |
|----------|-------------|---------------|----------|-----|--------------------------|--------------------------|
| 0x14 が ACK | ILI9881C | GT911（0x14） | 900Mbps | 80MHz | 140/40/40 | 20/4/20 |
| 0x55 の FW 版数 1 | ST7121 | ST7123（0x55） | 900Mbps | 70MHz | 40/2/40 | 24/20/200 |
| 0x55 の FW 版数 3 | ST7123 | ST7123（0x55） | 1040Mbps | 80MHz | 40/2/40 | 8/2/220 |

**背面ラベルの型番で決めてはいけない。** 背面に ST7123 と書かれた個体（2026-08-20 実測）でも
タッチの FW 版数は 1 で、正しい設定は上表の ST7121 の行（900Mbps / 70MHz）だった。この設定で
表示・タッチとも正常に動作している。ラベルは一体型タッチを積んだ世代を指しているだけで、
M5GFX のパネルクラス名（ST7121 / ST7123）とは対応しない。判定は必ず FW 版数で行う。

**自動判定（実装済み）:** `tab5_probe_panel()` が PI4IO のリセット解除後、まだ IDF の
i2c_master バスを握っている間に判定する。手順は M5GFX の Tab5 検出（M5GFX.cpp）に倣う。

1. 0x55（ST タッチ）のファームウェア版数レジスタ 0x0000 を読む。**1 なら ST7121、3 なら ST7123**
   （背面ラベルの型番とは別物。上表の注記を見ること）。
2. 0x55 が応答しなければ 0x14（GT911）のアドレス ACK だけ確認する。応答すれば ILI9881C 個体。
   実読しないのは、GT911 の内部ポインタを進めてしまうため。
3. どちらも応答しない間は 10ms 待って最大 60 回まで繰り返す。ST タッチはリセット後
   応答可能になるまで数十 ms（実測およそ 50ms）かかるので、1 回のプローブでは取りこぼす。
4. 打ち切った場合は、ST タッチが未知の版数で応答していれば ST7123（現行の量産品）、
   何も応答しなければ ILI9881C（従来の構成）とみなし、いずれも警告を出す。

判定を DSI ID ではなくタッチ側で行うのは、**ST7121 と ST7123 は lane 速度が異なり、
Bus_DSI を init する前に区別する必要がある**ため（M5GFX も同じ理由で touch を先に見る）。

判定結果は `LGFX_Tab5::configure()` に渡し、Bus_DSI の lane_mbps・Panel クラス・
DPI タイミング・Touch クラスを実行時に選ぶ。パネルは Panel_DSI 派生、タッチは
ITouch 派生で揃うので、描画経路（pushImage / getFrameBuffer）と
`display_p4_get_touch()`（`g_lcd.getTouch()`）は個体によらず不変。

注意: `tab5_power_on()` は GT911 のアドレスを 0x14 に確定させるため TP INT(GPIO23) を
リセット中 HIGH に駆動している（M5GFX は input のまま）。ST 個体ではこの HIGH 駆動は
本来不要で、ST7123 のタッチは 0x55 固定。実測では ST 個体でもこの駆動のまま FW 版数を
読めており、タッチ操作にも影響はなかった。

**実機確認 (2026-08-20, 背面ラベル ST7123 の個体):** 判定は `panel probe: ST touch FW
version 01` → `ST7121 + ST7123 touch`。LGFX init OK (1280x720)、Guru/abort なし、
実機の表示とタッチ操作（カーソル移動・クリック）ともに正常。

**ILI9881C 個体での退行確認 (同日):** `panel probe: ILI9881C + GT911 touch`。GT911 は
1 巡目の ACK で確定するので待ちは実質発生せず（power on からLGFX init まで 26ms）、
表示・起動とも従来どおり。ST 個体の判定には約 150ms かかった。
