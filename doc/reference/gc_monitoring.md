...
- **アイドル時に GC を進める仕組みは `FmrbApp#idle_gc=` で有効化できる**
  (3.2)。MIDI 系アプリと desktop が使っている。有効にした VM では GC は
  `_spin` のアイドル時間に進み、割り当て時には走らない。**有効にしていない
  VM では従来どおり、止まるのは必ず何かを割り当てた瞬間**である。
...
**自 VM の使用率だけ知りたいなら `FmrbApp.pool_usage`** (% を fixnum で
返す、無割り当て)。ポーリング用はこちら。`FmrbApp.ps` は 1 プロセスあたり
11 キーの Hash (384B の実体配列) を作るので、毎秒呼ぶとそれ自体が churn に
なる — プロセス一覧が要る場合も、先に `FmrbApp.ps_gen` (状態遷移ごとに
増える素のカウンタ) を見て、変化した時だけ `ps` を呼ぶ。
...
### 3.2 `GC.scheduler_driven` = `FmrbApp#idle_gc=` (P7 で実用化)
...
アプリからは **`self.idle_gc = true`** (`fmrb-app.rb`) で入るのが正で、
`_spin` の C 側 (`app.c` の `spin_idle_gc`) が待ち時間にステップを回す。
MIDI 系アプリと desktop が使っている。

`idle_gc=` は 4 つをまとめて設定する。**個別に手で設定するときは
`GC.malloc_threshold` を忘れないこと** (下記の罠):

| 設定 | idle_gc= の値 | 意味 |
|---|---|---|
| `GC.scheduler_driven` | true | 割り当て時 GC 停止、アイドル点でステップ |
| `GC.debt_limit` | 4096 | 忙しいままの安全弁 (超過で割り当て側が 1 ステップ) |
| `GC.step_limit` | 512 | 1 ステップの上限。リズム保持アプリ向けの値。desktop は 2000 に上書き (500ms の広い待ち窓があるため) |
| `GC.malloc_threshold` | 32 KB | **これが要**。下記参照 |

**罠 (2026-08-06 に desktop で踏んだもの)**:

- **`malloc_threshold` を設定しないと、「オブジェクト数は少ないのに
  消費バイト数だけが増える」割り当てを GC が一切検知できない** (初期値
  0 = 無効)。ステップ発動の判定 (`mrb_gc_scheduler_pending`) はオブジェクト
  数の負債しか見ておらず、Hash の実体配列 (1-3 キーで 128B、
  `hash_new_capa(12)` で 384B) や String バッファのような **malloc で
  確保される中身のバイト数は数えられない**。
  churn が Hash/String 主体のアプリでは回収がほぼ起きず、スロット不足も
  回収でなく**ヒープ追加 (~20.5KB/ページ) で逃げる**ため、プールが天井まで
  単調に伸びる。
- 天井に着くと、アロケータの空振り → 緊急の `mrb_full_gc` がプール全域を
  同期でマークする「**GC ストーム**」になる。desktop (819KB プール) で
  数秒〜15 秒の停止として観測された。周期ダンプの VM Pools で Used が
  「約 470KB → 818KB へ数分で単調増加 → 一気に崩落」の**鋸歯**を描くのが
  ログ上の指紋である。
- 世代別モードとは併用できない (minor サイクルが分割できないため、
  scheduler_driven=true が世代別を切る)。
- **ステップはメッセージが来ると中断する** (`spin_idle_gc`)。イベントが
  絶え間なく来る状況 (30Hz のマウス移動、Launcher 連打) ではアイドル窓が
  消え、ステップが追いつかない。これは仕様で、対策は 3.3 のウォーターマーク。

効果の確認は 2 章の計測ビルドで `prof_sync_*` (同期停止) が `prof_step_*`
に移ったかを見る。

### 3.3 ウォーターマーク GC (バースト割り当ての天井回避)

アイドルステップは「アイドルがある」ことが前提なので、**バースト的に
割り当てる UI 操作 (Launcher の開閉など) はステップを追い越して天井に
届き得る**。その場合はアプリ自身が使用率を見て、フレーム境界で自主的に
collect する:

```ruby
# on_update 内、5 秒ごと
if @counter % 10 == 0
  usage = FmrbApp.pool_usage   # 自 VM プールの使用率 % (無割り当て)
  if usage >= 70
    Log.info("watermark GC at #{usage}%")
    GC.start
  end
end
```

- `FmrbApp.pool_usage` は fixnum 1 個を返すだけで**測ること自体がゴミを
  作らない** (`FmrbApp.ps` は 1 プロセスあたり 384B の Hash を作るので
  ポーリングには使わないこと)。
- desktop 実測: これでプールは最大 568KB (69%) で頭打ちになり、ストームは
  消えた。**残るのはウォーターマーク発動時のフル GC 1 回分の停止**で、
  そのコストは主に生存オブジェクトのマーク量で決まる。**しきい値を下げても
  1 回の停止は短くならない** (生存集合は変わらない)。発動頻度を下げたいなら
  割り当てを減らすしかない (7 章)。
- 発動ログを 1 行入れておくと、後から「あの引っかかりはウォーターマークか」
  がログだけで判定できる。
...
| `FmrbApp.ps` を毎秒呼ぶ | プロセス数 x (Hash+384B) | `FmrbApp.ps_gen` で変化した時だけ呼ぶ |
| 周期ポーリング用の Hash 返し API (`wallclock` 等) | 1 + 実体配列 | 値 1 つで足りるなら bool/fixnum 返しの API を足す (`wifi_connected?` 方式) |
| 高頻度イベントごとの Hash 生成 (C 側) | 1 + 128B | 同一キー構成なら GC ルート付きで 1 個を使い回す (マウス移動の `$fmrb_move_ev` 方式。**受け側がイベントをまたいで保持しない**ことが前提) |
| 周期描画で文字列を組んで draw_text | 2〜3 | 整形と送信ごと C メソッドに降ろす (`Graphics#draw_wallclock` 方式)。desktop はこれで定常割り当てが**厳密にゼロ**になった (live/malloc_increase が 3 分間 1 も動かないことを C 側トレースで実測、2026-08-06) |
...
- desktop の GC ストーム (鋸歯 → malloc_threshold + churn 削減 +
  ウォーターマークで解決した事例): コミット `a862aba` と `9781201`。
  2026-08-06 の Tab5 実機ログで、819KB プールの鋸歯・15 秒ストール・
  568KB 頭打ちまでの一部始終を計測した
