...
|---|---|---|
| 1 件あたり平均 | 0.414 ms | 集計行が出ず (下記) |
| 最大 | 14 ms | **68〜90 ms** が繰り返し |
| 25 ms 超え | **0 件** | 記録された警告だけで 14 件 |

mruby 版は `hid_event slow: subtype=3 capture= 73ms` 相当の警告が操作中ずっと出続ける。
マウス移動は 30Hz (33 ms 間隔) なので、**1 件 70〜90 ms では処理が追いつかず
カーネルの待ち行列に溜まる**。Spinel 版はこの警告がゼロ。

読み方の注意 2 点:

- mruby 版は `hid_lat:` の集計行 (1000 件ごと) が**出ていない**。1 件が重いぶん同じ
  時間で処理できた件数が少なく 1000 件に届かなかったため。mruby 側は平均値ではなく
  「遅い件の実測値」しか無い。
- 測っているのは経過時間なので、その間にタスクが他へ譲った時間も含む。mruby
  カーネルは仮想機械の中で動き処理の途中で譲るので、70〜90 ms の一部は計算では
  なく順番待ち。ただし「入力が溜まる」という結果は変わらない。
...
- mruby カーネル側の `hid_lat` 集計行 (平均値)。マウスを動かし続けて 1000 件
  貯める必要がある
