...
**mruby カーネルとの比較を取得** (2026-07-30、同一コード。`b516abc` と `3ac50e3` の
差分は doc / .github / .env のみでファームのコードは同一。desktop はどちらも mruby、
操作手順も同じ: JA Text → Monitor → Piano を起動しウィンドウを動かす)。

| 区間 | Spinel カーネル | mruby カーネル |
|---|---|---|
| 平均 (アプリ起動・ドラッグを含む) | **0.414 ms** | 2.62 / 2.76 ms |
| 最大 (同上) | **14 ms** | 89 / 84 ms |
| 25 ms 超え (同上) | **0 件** | 16 / 15 件 |
| 平均 (マウス移動のみの静かな区間) | 未計測 | 1.16 ms |
| 最大 (同上) | 未計測 | 3 ms |

mruby 側は 1000 件の窓が 3 回出た。上 2 つがアプリ起動とドラッグを含む窓、3 つ目が
マウスを動かすだけの窓で、**同じビルドで平均 2.7 ms / max 89 ms と平均 1.16 ms /
max 3 ms に分かれた**。差が出るのは山の側で、素の 1 件あたりではない。

- **通常時の差は 2〜3 倍**。Spinel は 1 件 1 ms 未満が大半 (`ge1=362`)、mruby は
  静かな窓でも全件 1 ms 以上 (`ge1=1000`) で平均 1.16 ms。Linux の 2.0 対 4.8 ms と
  同じ桁で、これが素の実行速度の差。
...
- GC 停止時間の分布 (特に max)。Linux では AOT が有利だったが実機は未確認。
  **上の 70〜90 ms の山の正体がこれかどうかを決められるのはこの計測**なので、
  未計測項目の中では優先度が上がった
- Spinel カーネルの静かな窓 (マウス移動のみ)。mruby 側は取れているので、
  素の 1 件あたりの差を同条件で言うにはこちらも要る
- フラッシュ使用量
...
**実機で時計が合っていないことを確認 (2026-07-30、S3、Spinel カーネル)**。
...
**実用上の注意**: `.env` の既定が `FMRB_KERNEL_ENGINE=spinel` になったので、
**この 2 件を直すまで、実機ビルドは時計が合わずファイル同期も行われない**。
