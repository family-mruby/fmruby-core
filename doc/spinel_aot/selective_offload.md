...
- **オフロードでは別領域が要る** (アプリの mempool 枠は mruby VM が使用中)。
  **方針 (ユーザ決定 2026-08-04): アプリの mempool 枠から切り出して
  Spinel に与える**。新しい静的枠も PSRAM の動的確保も持ち込まず、
  アプリの総予算は据え置きのまま、オフロードを使うアプリが自分の枠から
  払う。アプリ単位のメモリ会計 (ps の表示も) が崩れない。
  - 含意 1: 切り出しは **spawn 時** に行う必要がある (mruby の est が
    枠全体を覆う前に分割する)。オフロードを使うことはリンクで確定して
    いるので、spawn 時に判定できる。
  - 含意 2: 「入力を見てから作る」は**枠内での作り直し**になる
    (切り出し領域の上限はアプリ枠の残りで頭打ち)。上限を超える入力は
    作成前に Ruby のエラーとして返す。
  - 含意 3 (**サイズの制約、実測に基づく**): アプリのソースコンパイルは
    アプリのプールから割り当てる (mrc_ccontext_new がアプリの mrb で
    作られる)。実測で **10.5 KB のソースが 500 KB プールの 308 KB を消費**
    (doc/midi/report/p6.md 6 章)。したがって 500 KB 枠からの切り出しは
    **100〜150 KB 程度が安全上限**。SmfCore (64 KB 級) は収まる。
    将来 bytecode キャッシュ (コンパイル済み .mrb をソース CRC キーで
    保存し再利用) が入ればコンパイルピークごと消え、この上限は大きく緩む。
...
**例外: 比較モード (gem 単位の opt-in、ユーザ要望 2026-08-04)**。
ベンチマーク・デモ用の gem は**両バックエンドを同時に積み、実行時に
切り替え**られる。排他の原則を破らずに済む鍵は名前空間で、同じ
`spinel/*.rb` を (1) Spinel で `RaycastCore` (ネイティブ)、(2) module で
包んで名前をずらした bytecode `RubyImpl::RaycastCore` の 2 回コンパイル
する。アプリは同じ API をダックタイピングで持ち替えるだけ
(`@core = @use_native ? RaycastCore : RubyImpl::RaycastCore`)。
**文字通り同じソースの実行時 A/B** が成立し、「同じ Ruby です」と
言い切れる比較・デモになる。既定は従来どおり排他 (両積みの flash 代は
比較 gem だけが払う)。bytecode 側のために `sp_export` は mruby では
no-op として定義しておく。第一号は raycaster (5.1)。
...
### 5.1 SmfPlayer 以外の候補 (2026-08-04 調査)

適性の基準: **バイト・数値のループが主体で、境界の出入りが平坦で小さい**こと。
逆に「結果が mruby オブジェクトの木になる」「1 件ごとに外を呼ぶ」は不向き。

| 候補 | 適性 | 根拠 |
|---|---|---|
| **エディタの構文ハイライト** | **高** | default/editor (Ruby 1,890 行) は「ファイルが大きいとハイライトを自動で切る」閾値を持つ = mruby で間に合っていない証拠。String -> 色区間列の平坦な純バイト処理。Modern「作る機械」の全画面エディタ戦略と直結し、閾値ごと消せる |

**エディタは方針決定済み (ユーザ、2026-08-04)**: 速度重視で改善していく
対象とし、**中核を editor-gem として切り出す。Ruby で書き続け、Spinel で
...
- **gem 側 (Spinel 対象)**: 文書モデル (@lines 相当) を gem の中に持たせ、
  編集は (操作, 位置, 文字列) のスカラーで流し込む。トークナイズ・検索・
  行折り返し計算は gem 内で行い、**可視範囲の色区間列・行テキストだけを
  平坦に返す** (SmfCore の「生存データは境界の向こうに置く」と同じ形)。
- **アプリ側 (mruby のまま)**: 描画、キー処理、ファイル I/O、UI 状態。
- エディタには on-device デバッガ (FMRB::Debug gem + 下分割 UI) が統合済み。
  gem 切り出しの際、デバッガ UI はアプリ側に残る (文書モデルとは独立)。
| **レイキャスト核 (raycaster.app.rb)** | **高** | `NUM_RAYS = 25` = mruby の速度に合わせて解像度を落とした痕跡 (エディタの閾値と同種の証拠)。固定小数点の純整数ループ + sin/cos 表引き。境界は極小 (入力 3 整数/フレーム、マップは初回 1 回、出力は列配列)。ネイティブ化で 160 本 = **絵が変わる**のでデモ映え最強。要 gem 切り出し (ユーザアプリ本体は AOT 不可)。次のボトルネックは描画側 (fill_rect の本数) なので 50-80 本で釣り合いを測る。なお現状は毎フレーム Hash x 25 を割り当てており (P6 と同じ病気)、Spinel 以前に P6 式で直せる。**比較モード (4.4) の第一号**: 両バックエンド同時搭載 + 実行時切替で、同一ソースの A/B を画面上で見せる (fps/処理時間の HUD を添える) |
| **SMF -> FMSQ の実機内変換** | 高 | 今はホスト専用ツール (tool/midi/smf2fmsq.rb)。走査が数 ms になれば「.mid を置くと実機で変換」が機能として成立。SmfCore の続編 |
| **将来の音声処理** | 高 (将来) | Tab5 (P4) は WROVER 無し。ミキシング・エンベロープ等は数値ループ + 割り当てゼロ必須 + GC 停止が即ノイズで、GC 境界の利点が最大に効く |
| JSON パース | 低 | 処理は純バイトだが結果が mruby の Hash/Array の木で、境界のオブジェクト生成が節約分を食う |
| BASIC | 対象外 | 実装が既に C++ (components/basic/core) |
| ユーザアプリ本体 | 対象外 | 実機上でコンパイルされるため AOT 不可 (仕組み上の境界) |
