...
| **型の確定しない Array の要素に `String` キーで `[]` する** (`entries = FmrbApp.config("shortcuts"); e = entries[i]; e["key"]`) と `unsupported call: node N (CallNode \`[]\`) recv=LocalVariableReadNode/tyNN argc=1 arg0ty6` でコンパイルが止まる。**要素の型が確定していれば動く**ので、詰まるのは配列の作られ方 | 空配列リテラル (`tables = []`) に `<<` で Hash を積むと要素型が確定せず、`[]` を出せない | まだ無い。`config` の戻りを `nil` でなく常に Array にしても直らない (nil 由来ではなく空配列リテラル由来) | **fork 課題 (未起案)**。`FMRB_APP_ENGINE_DESKTOP=spinel` の**唯一の残り**ブロッカー (2026-08-21。`system_desktop.app.rb#load_shortcuts` と `launcher.rb#launcher_exclude_dirs` が同型) | doc/ui_widgets/report/u2.md |
