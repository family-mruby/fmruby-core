...
| **`elsif` の条件にメソッド呼び出しを書く** (`elsif focusable?(target_pid)`)。**同じ呼び出しでも `if` / `unless` / `next unless` なら正常**。呼び出しが前置き文を必要とするとき (引数が GC ルートの要る値) だけ出るので、前置きの要らない呼び出しでは再現しない | `elsif` は C の `else { if (...) }` に落ちるが、生成器は `else` の中括弧の中に前置き文を出さず、`if (` の括弧内に落とす: `if (          sp_RbVal _t458 = lv_target_pid;` `SP_GC_ROOT_RBVAL(_t458);` `sp_..._focusable_p(...))` (gcc: expected expression before 'sp_RbVal')。上の `!` の行と同じ「前置きが式の中に落ちる」系で、**トリガが `!` ではなく `elsif` という文の位置**である点が違う | 条件の手前でローカルに退避する (`can_focus = focusable?(target_pid)` … `elsif can_focus`) | **fork codegen bug**。上の `!` の行と同一根因として B-1 に併記、修正 pending | 2026-08-24 サービスホスト S1 追補。`focusable?` を 4 か所から呼んだうち **`elsif` の 1 か所だけ**が落ち、`if` / `unless` / `next unless` の 3 か所は前置きを `if` の手前に文として出す正しい C だった (生成 C で確認済み) |
...
| **型の確定しない Array の要素を添字で取り出して `String` キーで `[]` する** (`e = entries[i]; e["key"]`) と `unsupported call: node N (CallNode `[]`) recv=LocalVariableReadNode/tyNN argc=1 arg0ty6` でコンパイルが止まる | 添字 `[]` で取り出した値には要素型が伝わらない。**ブロック引数には伝わる** (`entries.each do \|e\| e["key"]` は同じ配列で通る) | 起動時 1 回の経路なら `each` で受ける (`load_shortcuts`、2026-08-21)。hot path で `while` が要る場合は要素の型が確定する作り方 (typed array) にする | fork 課題 (未起案): 添字アクセスにも要素型を伝える | doc/spinel_aot/report/desktop_spinel_gap.md |
...
| **ローカル変数の名前が `rescue => e` の `e` と同じ**だと、同じメソッド内の `while (e = dir.read)` の `e` が Exception と推論され、`names << e` が例外の poly 配列になる。コンパイルは通り、**`names.sort` が実行時に segfault する** (比較不能の例外文字列を組む途中で落ちる) | ローカルの型はメソッド全体で名前ごとに 1 つ。rescue 変数と衝突すると勝つのは Exception | ループ変数を `ent` 等に改名する (file_manager / file_selector、2026-08-21)。**`rescue => e` のあるメソッドで `e` を他に使わない** | fork 候補: rescue 変数のスコープを rescue 節に限る | doc/spinel_aot/report/desktop_spinel_gap.md |
| **プログラムが参照する `FmrbConst::X` が生成モジュールに無い**と、コンパイルは通り、**初回参照時に NameError で VM が落ちる** (`KEY_F10` で最初の打鍵でデスクトップが終了した) | 未定義定数は静的に解決できず、raise を埋め込む | `gen_const_rb.rb` が `KEY_*` を const.c から全件生成するようにし、`gen_*_combined.rb` が未定義参照を**生成時に abort** するようにした (2026-08-21)。新しい定数を Spinel 側で使うときは生成器に足す | (fmruby 側の道具で塞いだ) | doc/spinel_aot/report/desktop_spinel_gap.md |
...
| **クラス階層を基底型の変数経由で呼ぶ (仮想ディスパッチ) は使ってよい**。`@pressed` (基底 Widget 型) に対する `activate` / `release` / `group`、サブクラス混在 Array からの `w.draw_widget(gfx)` は、いずれも `switch (recv.cls_id)` に展開されて正しく落ちる | Spinel は受信型の候補集合を静的に求め、候補ごとの分岐を生成する。**素直な継承は書き換え不要** (整数 kind + case のような手動ディスパッチに落とす必要は無い) | 確認済み 2026-08-21 (`FmrbUI` の probe。生成 C に `case 20: sp_Widget_activate ... case 23: sp_Stepper_activate` が出る) | doc/ui_widgets/report/u2.md |
| **poly 受信で呼ぶメソッドの名前は、プログラム全体で一意にする**。`draw` / `update` / `run` のような**ありふれた名前を poly 受信で呼ばない** | 上の候補集合は**メソッド名で引かれる**ので、poly 受信の `w.draw(gfx)` は**プログラム中の `draw` を全部**候補にして生成対象へ引きずり込む。その中に 1 つでもコンパイルできないものがあれば、**到達不能でもビルドが落ちる** (実例: `GfxBlock#draw(**kwargs)` が `unsupported hash literal (key/value type)` で全体を止めた。`SpriteImage#draw` も `yield` を使う同名メソッド) | `FmrbUI::Widget#draw` -> `#draw_widget` に改名して解決。**具体型受信なら起きない** (候補が 1 つに決まるため) | doc/ui_widgets/report/u2.md |
| (随時追記) | | |
