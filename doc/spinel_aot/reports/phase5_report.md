...
| 構成 | IRAM free |
|---|---:|
| mruby desktop | 163,372 |
| Spinel desktop (修正前) | 実質ゼロ (196 B の DMA 確保が失敗) |
| Spinel desktop (修正後) | 76,108 |
| + ユーザアプリ 1 (Kamon) | 50,548 |
| + ユーザアプリ 2 (Monitor) | 31,316 |

**mruby との差は約 87 KB 残っている**。その内訳はほぼ例外/catch スタックそのもので、
生成 TU 1 本あたり `sp_exc_*` 20,992 + `sp_catch_*` 21,312 = 42,304、2 本で 84,608 B。
深さを下げれば回収できるが、後述の理由で今回は見送った (Phase 6)。
