...
| codegen | **条件式の単項 `!` の下にある呼び出しで、前置き文が式の中に落ちる**。`if !run_path_allowed?(run_path)` が `if ((!  const char * _t228 = lv_run_path;  SP_GC_ROOT(_t228); sp_..._run_path_allowed_p(self, _t228)))` を出力し `expected expression before 'const'`。**`!` の有無が分岐点**で、同じ呼び出しでも `x && run_path_allowed?(y)` (statement-expr に落ちる) は正常。汎用バグ。上行 `286de9b` (sprintf の GC-root 初期化子混入) と**同じ「前置きが式の中に落ちる」テーマだが経路が別** (あちらは `:str` 引数位置、こちらは単項 `!` のオペランド)。最小再現は未作成 | 呼び出しの結果をいったんローカル変数に受け、それを否定する (`allowed = run_path_allowed?(x); if !allowed`)。fmruby-core `main/prebuild_scripts/kernel/fmrb_kernel.rb` の 1 箇所、理由をコメントで明記 | 2026-07-29 kernel-only Spinel ビルドで発現 (Linux)。`ruby_writing_constraints.md` B に対応行 |
...
| ディスパッチ | poly レシーバの `Array#index/find_index(x)` が dispatch されず raise (include?/rindex は対応済、index だけ欠落)。config `opts = s[:options]; opts.index(cur)` で **enum 変更時に desktop クラッシュ**。ストレージ種は実行時のみ判明 (homogeneous は StrArray/IntArray, mixed は PolyArray) | **fork 修正済 `dea18671`**: include? と同型で arg 型別に per-kind case (StrArray/IntArray/PolyArray) を emit + `sp_PolyArray_index` 新設 + 戻り型 nullable Int。test/poly_array_index.rb。test 1996/1 bench 58/0 | T4-5 / **DONE** |
| 記法 (Ruby 側) | poly `String#setbyte` が dispatch されない。原因は `ch = nil; ...; ch = "\x00"; ch.setbyte(...)` の **nil-init で ch が poly 化** (Integer#chr 代替の 1-byte 組立)。text 入力経路 | **Ruby 修正** (fork でなく): 具体型ローカル `c = "\x00"; c.setbyte(...); ch = c` に。setbyte の mutation は poly-box 越しが awkward なので Ruby 側が正 (ruby_writing_constraints C 節)。system_desktop.app.rb:372 | T4-5 / **DONE** (Ruby) |
| ディスパッチ | poly レシーバの `Integer#to_s(base)` (1-arg) が dispatch されず raise (argc==0 の to_s は poly-to-string で処理済、base 付きだけ欠落)。msgpack `unknown prefix 0x#{c.to_s(16)}` (poly getbyte バイト) | **fork 修正済 `b8bc2522`**: SP_TAG_INT pre-arm → `sp_int_to_s_base` + TY_STRING 推論。byteslice と同型。test/poly_int_to_s_base.rb。test 1997/1 bench 58/0 | T4-5 / **DONE** |
| (要修正でない) | `rtc.write_time` の raise は生成 C に残るが、**`if FmrbConst::PLATFORM == "esp32"` で囲われた dead code** = Linux では実行されない (RTC 書込は ESP32 のみ)。ESP32 実機では RX8900/RX8130 が別 mrbgem で Spinel desktop TU 外 → **Phase 5 の cross-gem 課題** (Spinel から別 gem のメソッド呼び出し)。Linux の poly-dispatch gap ではない | Linux=対処不要。Phase 5 で cross-gem 呼び出し方式を検討 | T4-5 / (Phase 5) |
