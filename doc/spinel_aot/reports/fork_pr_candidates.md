...
| ディスパッチ (残・latent, 非クラッシュ) | 生成 C に raise が残るが両方 non-fatal: **poly `Integer#to_s(16)`** (msgpack_pure.rb:270 の unknown-prefix エラーメッセージ = 不正 msgpack 時のみ)、**`rtc.write_time`** (clock 設定保存, `begin/rescue` 内で catch 済=非致命, Linux で rtc poly)。sweep 手法=生成C の `sp_nomethod_msg_args("X"` 全列挙 | 未対応 (低優先)。to_s(base) は poly-Int dispatch、write_time は Linux で RTC 書込を guard する Ruby 修正が筋 | T4-5 |
