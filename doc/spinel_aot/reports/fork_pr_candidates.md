...
| codegen | **条件式の単項 `!` の下にある呼び出しで、前置き文が式の中に落ちる**。`if !run_path_allowed?(run_path)` が `if ((!  const char * _t228 = lv_run_path;  SP_GC_ROOT(_t228); sp_..._run_path_allowed_p(self, _t228)))` を出力し `expected expression before 'const'`。**`!` の有無が分岐点**で、同じ呼び出しでも `x && run_path_allowed?(y)` (statement-expr に落ちる) は正常。汎用バグ。上行 `286de9b` (sprintf の GC-root 初期化子混入) と**同じ「前置きが式の中に落ちる」テーマだが経路が別** (あちらは `:str` 引数位置、こちらは単項 `!` のオペランド)。最小再現は未作成 | 呼び出しをローカルへホイストしてから否定 (`allowed = run_path_allowed?(x); if !allowed`)。fmruby-core `main/prebuild_scripts/kernel/fmrb_kernel.rb` の 1 箇所、理由をコメントで明記 | 2026-07-29 kernel-only Spinel ビルドで発現 (Linux)。`ruby_writing_constraints.md` B に対応行 |
