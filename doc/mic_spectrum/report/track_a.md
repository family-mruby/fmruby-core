...
- **ただし Spinel は C の 23 倍遅い**。ホスト (下記) では 7 倍だったので、
  **実機では Spinel の分が悪くなる**。差はホストの巨大なキャッシュと、
  実機側の Float の箱がプール上に散らばること。全プールが PSRAM
  (`EXT_RAM_BSS_ATTR`) なので、C の連続配列はキャッシュに乗り、Ruby の
  オブジェクトグラフは乗りにくい。**切り分けは次の課題** (プールを内蔵 RAM に
  置いた場合の比較、固定小数点版)。
...
- **Spinel が実機で C の 23 倍になる理由の切り分け**: プールの置き場所
  (PSRAM か内蔵 RAM か) と Float の箱。ホストとの比 (7 倍) の差がヒント。
