...
**P5 完了 (2026-08-10、コミット f15ceeb / eb926d3 / 342e667、report/p5.md)
= 段階 6 到達**:

- FMRB_APP_ENGINE_EDITOR=spinel でエディタ全体が Spinel 動作。ソースは
  1 本、.env 既定は mruby のまま。デバッガは同名モジュール差し替え
  (editor/debug_pane.rb ⇔ spinel/editor_debug_stub.rb) で、本体に分岐なし。
  接点は dbg_* 16 フック (将来の別アプリ化の境界にそのまま使える)。
- editor-core は 純C (ec_*) / mruby 薄層 / Spinel シム の 3 層に再編。
  **EditorCore の FFI 宣言は共有 FFI と別ファイルに分離が必須**
  (Spinel は全 ffi_func に extern を出すため、混成構成
  「エディタ mruby + デスクトップ Spinel」のリンクが壊れる) — 指示書に
  無かった落とし穴の発見。
- **書き方制約の修正は Integer#chr の 1 件のみ**。P1-P4 の書き換えで
  Spinel が苦手な形が先に消えていた (段階的改善の副次効果)。
- 実測: 打鍵→present は 10-20% 改善 (1.83→1.65ms 等。P4 で重い部分が
  既に C のため想定どおり小幅)。**本命はメモリ: VM プール使用率 68% → 15%**。
  S3 flash 残は 21% → 17% (パーティション再編計画の範囲内)。
- 標準構成で全シナリオ通過、互換構成でデバッガ含め退行なし、6 ビルド通過。
- 残: 実機確認 (P2-P5 まとめて)、段階 5 (640x360、実機とセット)、
  .env 既定を spinel に切り替えるかは実機確認後にユーザ判断、
  デバッガの将来形 (dbg_* フックの非同期メッセージ化が設計課題)。
