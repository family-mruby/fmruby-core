...
> 索引 577 件・39 クラス・78 ファイル・436KB。同名は受け手の型で選ぶ。
> F1 は 3 ms (1146 ms から)
...
ユーザ報告 (2026-09-02):「ブラウザ版でもヘルプを開くのに 1 秒以上かかる」。
どこで使っているかを測る `help_lat:` を入れた (`ti_lat` / `edit_lat` と同じ
考え方)。答えは一目だった:

```
help_lat: 1146 ms (index 1141, load 0, jump 0, hl 4; 674 lines)
```

**索引を引くところで全部**使っていた。読み込みも表示も 0 ms。
`text.split("\n").each { |l| l.split("\t") }` が 577 行ぶんの配列 577 個と
文字列 1154 個を作って捨てていた。

`String#index` の一発検索に変えた (`"\n<名前>\t"` を探す。前の改行と後ろの
タブが名前を挟むので、他の名前の末尾に一致することはない)。**1146 ms → 3 ms**
で、残る 3 ms は 674 行の markdown 強調。同名で受け手を聞く場合も 0 ms。

**教訓**: 端末側の Ruby では、**行に分けて要素ごとに分ける**形が数百行で
秒に届く。C の 1 回の検索で済むならそちらにする
([[feedback-draw-cost-scales-with-screen]] と同じ話が探索側にも当てはまる)。
