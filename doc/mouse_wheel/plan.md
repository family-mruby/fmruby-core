...
> 状態: 進行中 | 更新: 2026-09-01 | **W1 完了** (ブラウザ・sim で実測)、**W2 完了・実機未確認** (report/w2.md)。残りは W3 (実機 USB・白名簿) / W4 (nsf/smf と修飾キー)
...
| **W2** | ブラウザ (wasm の ring + ページ) と遠隔デスクトップ (rd_input + web クライアント) | **前半 (ブラウザ) は W1 と一緒に完了** — 検証がこの経路のため。**後半 (遠隔デスクトップ) も実装完了 2026-09-01** (report/w2.md)。TAB5 ビルドは通ったが、端末が繋がっておらず実機未確認 |
...
**追記 (2026-09-01)**: その後 sim も自分で確認した (Linux ビルドへ切替)。
中継 (graphics-audio) の型 switch にホイールが無く、そこで捨てられていた。
塞いだ上で log / ランチャーの実測まで取ってある (report/w1.md)。
