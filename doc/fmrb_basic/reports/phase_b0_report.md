...
```
--- FMRB_BASIC_SCRDUMP BEGIN tag=1 28x24 ---
T00 20 20 46 41 4D 49 4C 59 ... (28 バイトを 16 進で)
...
T23 ...
A00 |  FAMILY BASIC             |
...
A23 |                           |
C00 00001111222233330000111122222222
...
--- FMRB_BASIC_SCRDUMP END ---
```
...
- `C<行>`: 配色番号 (0-3) を 1 桁ずつ 28 文字。**既定では出力しない**
  ...
  使う。
- BG 面が 2 面ある (V3 SCREEN) 場合はヘッダに面番号を足す。
...
| 3 | `IF 式 THEN 文` の後に `:` で続く文が、条件不成立時にスキップされるか (MS 系 BASIC の挙動) | ベーマガ作品の分岐が丸ごと変わる | B1 で MS 系挙動 (行末までスキップ) を暫定採用予定。要ユーザ確認 |
| 4 | PLAY が非同期か (プログラム実行と並行して鳴り続けるか)、プログラム終了後も鳴り続けるか。spec は MOVE については「プログラム終了後も動く」と明記するが PLAY には記述がない | T0-4 の方式選択 (sec 4.3 は非同期前提) | 非同期を前提に設計。要ユーザ確認 |
