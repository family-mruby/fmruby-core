...
書き込み前に `fmrb_rd_ps` を単独で叩いたところ Monitor と SpriteEditor が
動いており、ユーザが操作中と見て一度中断した。許可を得てから実施している
(書き込みの再起動でその 2 本のセッションは終わっている)。

File Manager → `/home/slides` → `demo.md` をダブルクリック → **メニューを
飛ばして 1 枚目から発表が始まる**。`ps` で `PicoRabbit+` が RUNNING、
desktop は SUSPENDED (全画面に入っている)。

![Tab5 で .md をダブルクリックした結果](a1_tab5.jpg)

確認後 Ctrl+Q で閉じ、実機は元の状態 (kernel + desktop + Services) に
戻してある。

## まだ無いもの

- 無し (Spinel エディタの既存不具合は上記のとおり別タスク)。
