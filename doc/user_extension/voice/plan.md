...
- 解決の順: **(1) キャッシュ** `/tmp/tts_<鍵>.wav`
  (鍵 = server + speaker + text のハッシュ)、**(2) ローカル VOICEVOX**
  (素の HTTP)。**(3) クラウド TTS** (V2b、下記)。
  取得したら**キャッシュへ書いてから**鳴らす (一時名 → rename なので、
  途中で切れても半端なファイルを鳴らさない)。
- **キャッシュは PSRAM (`/tmp`) にあり、電源を切ると消える**。「同じ文は
  2 回目からオフラインで即時」は**同じ稼働の中での話**で、再起動すると
  もう一度合成しに行く (flash に 200KB の音声を書き続けないための判断。
  V2b の報告参照)。
- **恒久にオフラインで鳴らしたい定型文は、従来どおり `/home/voice/` に
  WAV を置いて `play_wav` で鳴らす** (V1 の運用)。tts のキャッシュは
  「同じ文を言い直すときの短縮」であって、置き場所ではない。
  電源を切っても鳴ってほしい音 —  時報、アラーム、起動時のあいさつ —
  はこちら。
