...
| `components/fmrb_common/include/fmrb_task_config.h` | `FMRB_SERVICE_APP_PRIORITY (1)` と `FMRB_SERVICE_APP_TASK_STACK_SIZE` を新設 (実測後 20KB) |
...
`FMRB_SERVICE_APP_TASK_STACK_SIZE` はこの実測を受けて **24KB → 20KB**
(bf95479)。山を作るのは mruby の codegen の再帰で、これは**サービスの本数では
なく 1 本の入れ子の深さ**で決まる (ユーザが深い Ruby を書けば伸びる) ため、
10KB の余地を残して 4KB を内蔵 RAM に返した。**上げ直すのは実際に溢れたとき
だけ** (スタック保護フォールトがタスク名を出す) で、勘で盛らない。
20KB で再確認して high-water は 10,128 B のまま。
