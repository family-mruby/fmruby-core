...
> 状態: 進行中 | 更新: 2026-09-01 | P0-P3 完了 (P3 は実機確認 4 件残)。P4 完了 (report/p4.md)。**P6 = 青ちらつきに帯域削減で当たる計画を追加** (instruction_p6.md、未着手)。残り = ユーザ確認とモニタ相性、無印 ESP32 疎通 (保留)
...
### P3: 無線・入力・周辺の差し替え (実装完了 2026-08-30、report/p3.md)
...
### P6: 走査帯域を減らしてアンダーランを緩和する (計画済、instruction_p6.md)

起動時・重負荷時の青ちらつき (P4 の項目 6) に、**需要を減らす側**から
当たる。report/p4.md に残した 3 つのリードは全部「供給を増やす」側で、
この 4 つ目はそこに載っていない。高解像度も高 FPS も要件ではない
(ユーザ判断 2026-09-01) ので、走査 DMA が毎秒要求する 178.7MB/s 自体を
削れる。

- **B1 ピクセル形式 RGB565** (-33%、DSI リンクの 96% 占有も同時に解ける)。
  「ブリッジは RGB888 のみ」は測った話ではないと確認済み。
- **B2 ピクセルクロック 60MHz = 48.5Hz** (-25%、変更 2 行)。
- **B3 計測**が本体。帯域に比例して件数が減るのか、瞬間風速だけの問題
  なのかを決める (後者ならリード 2 が本命と分かって撤退できる)。

P5 は実基板用に予約済みなので番号を飛ばしている。
...
- **マイクゲインの適値** (音は入ることを確認済み。30dB が効いたかは同じ音源で
  0dB と比べていないので未確定。report/p3.md)。47160Hz は ES8311 の係数表に無く、
  48000 の係数で開いてからリンクを 47160 に戻す形で解決済み。
- **起動時・重負荷時の一瞬の青ちらつき** (DSI アンダーラン)。**帯域を
  減らして当たる計画を instruction_p6.md に起こした (2026-09-01)**。機構確定・
  定量観測可能・当面許容。試行済みの対策と残るリードは report/p4.md 課題 6。
