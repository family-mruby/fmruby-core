...
{
  if (ptr) {
    /* family-mruby (pr#2): close unconditionally (with the new vm-threaded
     * signature). A socket that merely looks "closed" (state != CONNECTED after
     * a peer EOF/error) still holds its mbedTLS session and fd; upstream's
     * `if (!SSLSocket_closed) close` guard leaks those. SSLSocket_close is safe
     * to call in any state, and an explicitly-closed socket has DATA_PTR=NULL so
