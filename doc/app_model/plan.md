...
> 消し、基底が予約する名前を 15 個減らす**。アプリに触るのは 6 本だけ
...
| **M2** | B の改名 13 個 + アクセサの手書き化 + `editor/menu.rb` | 標準構成でデスクトップ・エディタ・シェルが起動し、全画面の出入りができる |
| **M2b** | `@running` `@name` を `@_` へ。直接読んでいる **6 本**を `running?` / `name` に置換 | shell が動き続ける (`while running?`)、kamon / stackchan 2 本が止まらない、窓のタイトルが正しい |
