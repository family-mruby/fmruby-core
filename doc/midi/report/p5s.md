...
## 7. 実機での計測 (2026-08-04 追記)

S3 実機 + Unit MIDI で動作確認した際に、**sim では見えなかった実機固有の
遅さ**が 2 つ出た。原因を数字で押さえたので記録する。
計装はデモアプリと `SmfPlayer#timing_stats` に入っている。

### 7.1 実機で確認できたこと

| 項目 | 結果 |
|---|---|
| UART2 が空いているか (P0-3) | **空いていた**。`midi_serial: MIDI serial out ready (path=- uart=2 tx=47 baud=31250)` |
| GROVE 端子2 の TX が GPIO47 か | **推定どおり**。入れ替え不要 |
| Unit MIDI が鳴るか | **鳴った** (5V 給電が要る) |
| APU 経路 (P2/P4) が実機で動くか | **動いた** |

**sim で作り切ったものが実機で 1 行も直さずに動いた**。P5-sim の狙いは
達成できている。

### 7.2 「ボタンを押してから何秒も待つ」の正体
...
**ファイル読み込みは 31ms、`channel_usage` のフルスキャンが 7.9 秒**である。
`channel_usage` は曲全体を 1 バイトずつ可変長デコードして走るので、
実機の mruby ではここが支配的になる。**sim では一瞬で終わるため気付けなかった**
(桁が 2 つ違う)。

対策の候補:

- **先頭 N イベントだけ走査**して声を決める (使用チャンネルは曲の冒頭で
  ほぼ確定する)。最も安い。
- **`collect_tracks` の 1 パスに相乗り**させる。今は読み込みと走査で
  ファイルを 2 回舐めている。
- C に降ろす。

### 7.3 テンポ不安定は「常時」ではなく「たまに詰まる」

The Entertainer を 60 秒ほど再生した実測 (APU 経路でも serial 経路でも
同様に不安定):
...
- **平均遅れ 10ms** は 16 分音符 (約 100ms) に対して聞き取れない水準。
- 効いているのは **176ms の単発スパイク**と、**200ms を超えて
  スケジュールをずらした回数 (stalls) が 4 回**のほう。
- したがって「Ruby のデコードが常時間に合っていない」のではなく、
  **時々長く止まる**形である。GC が疑わしいが、**まだ裏は取れていない**
  (GC の統計は採っていない)。

次に測るなら **GC の回数と停止時間**。プレーヤは 1 イベントごとに
文字列や配列を作るので、割り当てを減らせるかも併せて見る。
