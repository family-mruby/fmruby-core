...
**GROVE 端子2 は 5V / 3.3V を選択して供給できる**。Unit MIDI は
**5V 側でないと鳴らない**。別途給電は要らないので、ケーブル 1 本で完結する。

**最初に踏んだのがここである** (2026-08-04)。UART は正常に開き
(`midi_serial: MIDI serial out ready (path=- uart=2 tx=47 baud=31250)`)、
書き込みエラーも出ないのに無音、という状態になった。**ログ上は何も
異常が無いので、ソフト側を疑うと時間を使う**。外部音源が鳴らないときは
まず給電 (5V が選択されているか、ユニットの LED が点いているか) と、
**ユニットの 3.5mm ジャックに何か挿さっているか** (Unit MIDI に
スピーカーは内蔵されていない) を先に見ること。
