...
逆に USB-MIDI **host** と UART(DIN) MIDI は両ボードで成立しうる。
...
2. UART(DIN) MIDI の外部機器 / SAM2695 のような音源チップ
3. **内蔵の APU**
...
- Family mruby BASIC の `PLAY`: MML を事前に FMSQ へ変換する方式
  (`basic_mml.cpp` にテンポとオクターブの表がある)。
- `picoruby-midi-mml`: 実時間で解析して MIDI イベントを送る方式。

共通化の形としては、**中間イベント列を定義して前後を分ける**のが素直:

```
[BASIC MML 方言]  ─┐                        ┌─> [FMSQ 書き出し]
                   ├─> 中間イベント列 (音高/長さ/テンポ/音色) ─┤
[picoruby MML 方言]─┘                        └─> [MIDI transport へ送出]
```

前提として、**両方言の差分表を作るのが先**。テンポの基準 (BASIC は T4 =
4 分音符 0.5s)、オクターブの基準 (BASIC は O3 の C = 中央ハ)、音長・タイ・
音色指定の記法がどこまで一致するかを洗い出す。差が小さければ片方に寄せられるし、
大きければ「同じ中間表現に落とす 2 つの前段」として残す。
...
|---|---|---|---|
| APU (内蔵) | 可 | 可 | 方向B |
| **UART(DIN) MIDI 出力** | **GROVE 端子2 (GPIO47/48) で確定** | 空き GPIO (割り当ては後で決める) | 下記 E-1。最初の外部出力はこれ |
| USB-MIDI host | 可 (要改造) | 可 (要改造) | 下記 E-2 |
| BLE-MIDI | 可 (BLE ドライバあり) | 要調査 | 将来。Midori 側も未実装 |

#### E-1: UART(DIN) MIDI 出力 (両ボード共通の第一の外部出口)
...
- TX / RX のどちらを GPIO47 に割り当てるかは、ESP32-S3 の GPIO マトリクス経由で
  ソフトウェア側から選べる。接続先の機器や自作の変換基板に合わせて後で決められる
  (配線のやり直しにはならない)。
...
- **MIDI OUT は軽い**。TX ピンから抵抗 2 本を介して DIN-5 (または TRS type A) に
  出すだけで、絶縁も外部電源も不要。Family mruby は送る側に徹するので、
  ここだけで用が足りる。
- MIDI IN は光結合が要る (受け側の回路が必要)。**当面は対象外**。
  ただし transport 抽象は入力も持っているので、後から足せる形にしておく。
- 通信速度は 31250 baud。

**すぐ効く副産物**

GROVE / Port A に UART MIDI が出るなら、**M5Stack の SAM2695 系の音源ユニットを
そのまま挿せる**可能性が高い (Midori が `picoruby-sam2695` で使っているのと同じ経路)。
配線を自作しなくても、外部の GM 音源で鳴らす形が最短で試せる。
これは方向B (内蔵 APU) と対になる「外部音源」の実例になる。
...
| 段 | 内容 | ハード依存 | 目的 |
|---|---|---|---|
| P0 | 計測と調査 | 実機/sim | APU 発音遅延**と和音連打のスループット**、FMSQ 再生中の note_on の挙動 (instance/声の奪い合い)、S3 の空き UART ユニット確認、gem 追加後の flash サイズ、MML 方言差分表 |
| P1 | SMF -> FMSQ 変換 (方向C-a) | 無し | 先に成果が出る。以降のテスト素材にもなる |
...
| P5 | **UART(DIN) MIDI 出力 (方向E-1)** | 実機 (配線のみ) | 外部機器を鳴らす第一手。まず S3 の GROVE 端子2 で通す |
| P6 | MML 共通化 (方向D) | 無し | BASIC と Ruby で同じ MML |
| P7 | USB-MIDI host 相乗り (方向E-2) | 実機 | 挿した機器を鳴らす。手数が多いので後 |
| P8 | アプリ (シーケンサ、パッド、MML プレーヤ) | - | Midori のアプリを移植 |

P0 の結果次第で P2 の実装深度 (Ruby だけで書くか、C 層に降ろすか) が変わる。
P5 は配線 (抵抗 2 本 + コネクタ) か市販の音源ユニットがあれば試せるので、
基板改版を待つ必要はない。
...
- MML を BASIC 側に寄せるか picoruby 側に寄せるか、中間表現で分けるか。
