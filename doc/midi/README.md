...
- **GROVE 端子2 が 5V を供給しているか**。Unit MIDI は 5V 電源が要る
  (信号は 3.3V ロジックで問題ない)。基板が 3.3V しか出していない場合は
  ユニット側に別途給電が要る。リポジトリ内に端子の電源仕様の記述が無いため、
  実機・回路図での確認が必要。
...
- **GROVE 端子2 の 5V 供給と結線の確認**。Unit MIDI は 5V が要る。
  黄/白がどちらの GPIO に来ているかも基板で確認する (E-1 参照)。
...
| P5 | **UART MIDI 出力 / Unit MIDI (方向E-1)** | 実機 (GROVE ケーブルのみ) | 外部音源を鳴らす第一手。S3 の GROVE 端子2 に Unit MIDI を挿す |
