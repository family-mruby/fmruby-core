...
- **壁紙が見える時間はもっと長い**。`spawn_startup_app` が呼ばれてから
  ...
  **この 1 点はもう当てにならない** (2026-09-03 追記)。店を default app へ
  移して bytecode にしたので、同じ操作が **0.45 秒**になった
  (app-distribution/report/builtin_move.md)。5 秒のほとんどは
  **その場での翻訳**で、`flash/app/**` に置いた Ruby 特有の費用だった。
  組み込みでないアプリを測り直すこと。
...
- **時限の値は 25 秒に揃えてよい**。「起動中」が出ているので長くても迷わない。
  ...
  確かめる (App Store の約 5 秒という 1 点は無効になった。上記参照)。
