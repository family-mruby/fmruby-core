...
> 状態: 進行中 | 更新: 2026-09-03 | 実装済み・**未検証**。`boot_splash` と、
> 全画面の `startup_app` を待つあいだの中央表示まで入れた。実機/sim で
> 動かしていない (検証には全書き込みか両リポジトリの Linux ビルドが要る)。
...
- **未検証**。動かしていない。確かめるべきは受け入れ条件の 6 項目、特に
  **失敗したときにデスクトップが出ること** (`.app.toml` を故意に壊す)。
