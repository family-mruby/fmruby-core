...
> 状態: 計画済 | 更新: 2026-09-03 | 設定 1 個で「1 つのことをする機械」に
> なる。方針は確定 (キー名 `boot_splash`、待ち時間は画面中央に「起動中」)。
> 実装は未着手。
...
- **文言**。「起動中」か、アプリ名を出すか (`起動中: App Store`)。指示器の
  既存の書式と i18n (`system_desktop/i18n.rb`) に合わせて決める。
