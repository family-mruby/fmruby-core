...
`report/p1_p4.md`。**P1〜P5 は Tab5 実機で確認まで済んでおり、この計画は
完了とする** (SD の動画が窓の中で 14.77 コマ/秒、全画面 416x240 なら 30 コマ秒
まで通る)。残しているのは Spinel からの再生の確認だけで、これは
**当面やらない** (Spinel アプリから動画を出す使い道を今は想定していない)。
...
PSRAM も元に戻る。**Spinel 側の結び付けは実装だけあって未確認**。Spinel アプリ
から動画を出す使い道を今は想定していないため、確認せずに置く (2026-08-20 判断)。

### P5: 測って詰める

→ **完了** (2026-08-20)。数字は `report/p1_p4.md` の 9。要点:

- 段階の内訳は **読み > 写し >> 復号**。復号は全画面でも 3ms 弱で、
  ハードウェア復号器には余裕がある。効くのは canvas への写しで、動画の
  大きさではなく **canvas の広さ**に比例する (確定用の面へ canvas 全体を
  写すため)。読みは媒体で変わり、揺れが大きい (SD で平均 8ms・最大 21ms)。
- 上限は **416x240 / 30 コマ秒**。幅は 16 の倍数という復号器の条件があるので、
  426 幅の画面では 416 が全画面にあたる。30 コマ秒では合成が 33ms 間隔なので
  表示側が 7〜11% 取りこぼす。滑らかさを取るなら 15 コマ秒。
- 内蔵メモリの増分は P1 の時点で +3KB、再生中の空きは 135,900B (待機時
  155,060B)。
- 常設の計装を入れてある: 再生中に `p4_video: profile ...` が 60 コマごとに
  読み / 復号 / 写しの平均と最大を出す。
