...
ユーザがブランチ先端 (Phase C まで適用済み) を ESP32-S3 実機に書き込み、
動作することを確認 (2026-08-01)。headless 検証では見られない範囲
(実機の描画・入力・音声) がこれで埋まった。Phase A の HID 予約枠の保護、
Phase B のコマンド組み立て、Phase C の canvas 解放と HID 解読は、いずれも
実機のパスを通っている。
