...
startOverlay.addEventListener('click', async () => {
  ...
  // The store list is drawn before the machine exists, so nothing was
  // installed as far as it could tell. Draw it again now that the files are
  // there to look at.
  if (typeof storeRender === 'function') storeRender();
  hookInput();
  requestAnimationFrame(paint);
  if (ac) {
    try {
...
// The store.
//
// The list and the apps come from family-mruby-apps over https; the page does
// the fetching because the browser build has no networking of its own (no BSD
// sockets, so no Net::HTTP -- lib/add/family_mruby_wasm.rb). Installing is
// writing files into /flash/app/usr, which is one of the two directories kept
// in IndexedDB, so an app stays put across reloads.
//
// What the page cannot do is start one. There is no debug server here and no
// startup-app setting, so a freshly installed app waits for the next reload,
// when the machine scans /app again and the launcher lists it.

const STORE_BASE_DEFAULT =
  'https://raw.githubusercontent.com/family-mruby/family-mruby-apps/main/';
// ?registry=<url> points the store somewhere else, which is how it gets tested
// without pushing to the published list. Either half of the pair works: a
// directory to look for registry.json in, or the file itself (which is what
// you have to hand when trying a variant of the list side by side with it).
const [STORE_BASE, STORE_REGISTRY] = (() => {
  const q = new URLSearchParams(location.search).get('registry');
  if (!q) return [STORE_BASE_DEFAULT, STORE_BASE_DEFAULT + 'registry.json'];
  if (q.endsWith('/')) return [q, q + 'registry.json'];
  if (q.endsWith('.json')) return [q.replace(/[^/]*$/, ''), q];
  return [q + '/', q + '/registry.json'];
})();

// The pools a wasm build gets. CONFIG_IDF_TARGET_LINUX is defined for it, so
// it inherits the simulator's sizes (fmrb_mem_config.h).
const STORE_POOL_KB = 1536;
const STORE_POOL_LARGE_KB = 3072;

const storeList = document.getElementById('store-list');
const storeState = document.getElementById('store-state');
const storeRefresh = document.getElementById('store-refresh');
const storeReload = document.getElementById('store-reload');

let storeRegistry = null;

async function storeFetchRegistry() {
  const res = await fetch(STORE_REGISTRY, { cache: 'no-store' });
  if (!res.ok) throw new Error(`the list answered ${res.status}`);
  const reg = await res.json();
  if (!reg || !Array.isArray(reg.apps)) throw new Error('the list has no apps');
  return reg;
}

async function storeSha256(bytes) {
  const digest = await crypto.subtle.digest('SHA-256', bytes);
  return Array.from(new Uint8Array(digest))
    .map((b) => b.toString(16).padStart(2, '0')).join('');
}

// Fetch one file and check it is the one the list described. Size is checked
// too: it is free, and it catches a truncated read before the hash does.
async function storeFetchFile(app, entry) {
  const res = await fetch(STORE_BASE + app.base + entry.path, { cache: 'no-store' });
  if (!res.ok) throw new Error(`${entry.path}: ${res.status}`);
  const bytes = new Uint8Array(await res.arrayBuffer());
  if (entry.size != null && bytes.length !== entry.size) {
    throw new Error(`${entry.path}: ${bytes.length} bytes, expected ${entry.size}`);
  }
  if (entry.sha256 && (await storeSha256(bytes)) !== entry.sha256) {
    throw new Error(`${entry.path}: contents do not match the list`);
  }
  return bytes;
}

// What is on the machine already. The installed .app.toml is the record --
// app_id and app_version are in it, so there is no second ledger to keep in
// step with the files.
function storeInstalled(id) {
  if (!M) return null;
  const dir = `${APPUSR_PATH}/${id}`;
  let names;
  try { names = M.FS.readdir(dir); } catch (e) { return null; }
  const toml = names.find((n) => n.endsWith('.app.toml'));
  if (!toml) return null;
  let text = '';
  try {
    text = new TextDecoder().decode(M.FS.readFile(`${dir}/${toml}`));
  } catch (e) { return null; }
  const m = text.match(/^\s*app_version\s*=\s*"([^"]*)"/m);
  return { version: m ? m[1] : null, files: names };
}

// Whether this browser can run it at all: the app has to name web among the
// environments its author actually ran it in, and it has to fit the screen.
function storeBlockedReason(app) {
  if (Array.isArray(app.env) && !app.env.includes('web')) {
    return `not checked in a browser (the author ran it on ${app.env.join(', ')})`;
  }
  if (app.min_width && app.min_width > canvas.width) {
    return `needs ${app.min_width} px of width; this screen is ${canvas.width}`;
  }
  if (app.min_height && app.min_height > canvas.height) {
    return `needs ${app.min_height} px of height; this screen is ${canvas.height}`;
  }
  const kb = app.required_heap_kb && app.required_heap_kb.linux;
  if (kb && kb > STORE_POOL_LARGE_KB) {
    return `needs ${kb} KB of heap; the largest pool here is ${STORE_POOL_LARGE_KB} KB`;
  }
  return null;
}

// An app that does not fit the ordinary pool has to say so in its manifest,
// because the firmware's key is a boolean and means a different number on
// every machine. The size travels; the boolean is written here, for here.
function storeNeedsLargePool(app) {
  const kb = app.required_heap_kb && app.required_heap_kb.linux;
  return !!(kb && kb > STORE_POOL_KB);
}

function storeWithLargeMemory(bytes) {
  const text = new TextDecoder().decode(bytes);
  if (/^\s*large_memory\s*=/m.test(text)) return bytes;
  return new TextEncoder().encode(
    `${text}\n# Written when this app was installed: it asked for more heap than\n` +
    `# this machine's ordinary pool holds (spec.md 8.4).\nlarge_memory = 1\n`);
}

// The list and the files are fetched separately, so a push between the two
// shows up as a hash that does not match. Take the list again and try once
// more before calling it a failure (spec.md 13.5).
async function storeInstallWithRetry(app, say) {
  try {
    return await storeInstall(app);
  } catch (e) {
    say(`could not read it (${e.message}); taking the list again...`);
    storeRegistry = await storeFetchRegistry();
    const again = storeRegistry.apps.find((a) => a.id === app.id);
    if (!again) throw new Error('it is no longer in the list');
    return await storeInstall(again);
  }
}

async function storeInstall(app) {
  if (!M) throw new Error('switch the machine on first');
  const dir = `${APPUSR_PATH}/${app.id}`;
  const large = storeNeedsLargePool(app);

  // Fetch everything before writing anything, so a failure halfway through
  // the download cannot leave a half-installed app behind.
  const fetched = [];
  for (const entry of app.files) {
    let bytes = await storeFetchFile(app, entry);
    if (large && entry.path.endsWith('.app.toml')) bytes = storeWithLargeMemory(bytes);
    fetched.push([entry.path, bytes]);
  }

  storeRemoveFiles(app.id);
  M.FS.mkdirTree(dir);
  for (const [name, bytes] of fetched) {
    const slash = name.lastIndexOf('/');
    if (slash > 0) M.FS.mkdirTree(`${dir}/${name.slice(0, slash)}`);
    M.FS.writeFile(`${dir}/${name}`, bytes);
  }
  flushHome();
  return large;
}

// Remove the files and the directory. Leaving empty directories behind is
// harmless -- the machine's scan looks for a .toml and finds none -- but they
// accumulate in storage, and nothing else would ever clear them.
function storeRemoveFiles(id) {
  if (!M) return;
  const dir = `${APPUSR_PATH}/${id}`;
  let names;
  try { names = M.FS.readdir(dir); } catch (e) { return; }
  for (const name of names) {
    if (name === '.' || name === '..') continue;
    try { M.FS.unlink(`${dir}/${name}`); } catch (e) { /* a subdirectory */ }
  }
  try { M.FS.rmdir(dir); } catch (e) { /* not empty; leave it */ }
}
...
function storeRow(app) {
  ...
  img.alt = '';
  // This page is cross-origin isolated, so a plain <img> from another host is
  // only allowed when that host returns CORP. raw.githubusercontent.com does,
  // but GitHub Pages does not -- asking for it with CORS instead works on
  // both, since both return Access-Control-Allow-Origin.
  img.crossOrigin = 'anonymous';
  // Not lazy: the list is short, and a picture that has not loaded yet looks
  // exactly like one that failed.
  img.addEventListener('error', () => {
    img.replaceWith(Object.assign(document.createElement('div'),
      { className: 'app-shot app-shot-missing', textContent: 'no picture' }));
  });
  if (app.screenshot) img.src = STORE_BASE + app.base + app.screenshot.path;
  row.appendChild(img);
  ...
  meta.textContent = [app.category, `v${app.version}`, app.author,
                      (app.env || []).join('/')].filter(Boolean).join(' - ');
  const desc = document.createElement('div');
  desc.className = 'app-desc';
  desc.textContent = app.description || '';
  const note = document.createElement('div');
  note.className = 'app-note';
  body.append(name, meta, desc, note);
  ...
  const blocked = storeBlockedReason(app);
  const installed = storeInstalled(app.id);

  const say = (text, kind) => {
    note.textContent = text || '';
    note.className = 'app-note' + (kind ? ' ' + kind : '');
  };
  ...
  if (blocked) {
    ...
    say(blocked, 'warn');
    if (installed) {
      // It got here somehow (installed before the screen shrank, say), so
  ...
  function installButton(label) {
    ...
    btn.addEventListener('click', async () => {
      ...
      let large;
      try {
        large = await storeInstallWithRetry(app, say);
      } catch (e) {
        say(`could not install it: ${e.message}`, 'bad');
        btn.disabled = false;
        return;
      }
      storeRender();
      // storeRender rebuilt the row, so say() would write into a node that is
      // no longer on the page. Put the outcome where it survives.
      storeSay(app.id, 'installed. Reload to see it in the launcher.' +
               (large ? ' It was given the large memory pool.' : ''), 'good');
  ...
  if (!installed) {
    actions.appendChild(installButton('Install'));
  } else if (installed.version !== app.version) {
    say(`installed ${installed.version || '(unknown)'}, available ${app.version}`, 'warn');
    actions.append(installButton('Update'), removeButton());
  } else {
    say(`installed, v${installed.version}`, 'good');
    actions.appendChild(removeButton());
  }
  return row;
}

// A message about one app that has to survive the next storeRender (which
// throws every row away and builds them again).
const storeNotes = new Map();

function storeSay(id, text, kind) {
  storeNotes.set(id, { text, kind });
  const row = storeList.querySelector(`[data-app-id="${CSS.escape(id)}"] .app-note`);
  if (row) {
    row.textContent = text;
    row.className = 'app-note' + (kind ? ' ' + kind : '');
  }
}
...
function storeRender() {
  ...
  for (const app of storeRegistry.apps) storeList.appendChild(storeRow(app));
  for (const [id, note] of storeNotes) storeSay(id, note.text, note.kind);
  const n = storeRegistry.apps.length;
  storeState.textContent = `${n} app${n === 1 ? '' : 's'}`;
}

async function storeLoad() {
  storeState.textContent = 'loading the list...';
  storeList.textContent = '';
  try {
    storeRegistry = await storeFetchRegistry();
    storeRender();
    ...
    storeRegistry = null;
    storeState.textContent = `could not load the list: ${e.message}`;
  }
}

storeRefresh.addEventListener('click', storeLoad);
storeReload.addEventListener('click', () => location.reload());

// ?install=<id> installs one app as soon as the list and the machine are both
// ready, so a link can carry an app rather than instructions for finding it.
//
// It stops there. Starting the app would need something on the machine's side
// -- there is no debug server in the browser and no startup-app setting -- so
// the page says to reload, and the launcher has it after that.
async function storeAutoInstall() {
  const want = new URLSearchParams(location.search).get('install');
  if (!want) return;
  await storeReady;
  if (!storeRegistry) return;
  const app = storeRegistry.apps.find((a) => a.id === want);
  if (!app) { storeState.textContent = `no app called ${want} in the list`; return; }
  const blocked = storeBlockedReason(app);
  if (blocked) { storeState.textContent = `${want}: ${blocked}`; return; }
  // Wait for the machine: there is nowhere to write the files until it is up.
  for (let i = 0; i < 600 && !M; i++) await new Promise((r) => setTimeout(r, 100));
  if (!M) { storeState.textContent = `${want}: the machine did not start`; return; }
  const say = (text, kind) => storeSay(want, text, kind);
  say('installing...');
  try {
    await storeInstallWithRetry(app, say);
    storeRender();
    say('installed. Reload to see it in the launcher.', 'good');
    storeReload.hidden = false;
  } catch (e) {
    storeRender();
    say(`could not install it: ${e.message}`, 'bad');
