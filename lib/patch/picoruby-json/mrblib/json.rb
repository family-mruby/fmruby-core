...
module JSON
  ...
  # A wasm32 build does not have to have Regexp. Family mruby's browser build
  # is wasm32 and deliberately carries no regexp engine, so taking the fast
  # path on the strength of RUBY_DESCRIPTION alone raised "uninitialized
  # constant Regexp" out of JSON.parse -- on that machine only, and only once
  # something actually parsed JSON.
  def self.regexp_available?
    return @regexp_available unless @regexp_available.nil?
    @regexp_available = begin
  ...
  def self.use_regexp?
    ...
    @use_regexp = wasm_build? && regexp_available?
  end
