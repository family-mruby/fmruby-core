 ...
 *     GC.step -> nil
 *
 *  Run one bounded incremental collection step (mrb_gc_step). Exists for
 *  scheduler-driven mode: the C-side idle stepping stops whenever messages
 *  are pending, so an app under sustained input can outrun collection and
 *  hit its pool watermark, paying one long synchronous GC. Calling a few
 *  steps per frame from Ruby lets a busy app spread that cost into slices
 *  it can afford (family-mruby patch).
 */
static mrb_value
gc_step_m(mrb_state *mrb, mrb_value obj)
{
  mrb_gc_step(mrb);
...
mrb_init_task_gc(mrb_state *mrb)
{
  ...
  /* Plain (non-presym) registration: "step" may not be in the presym table. */
  mrb_define_class_method(mrb, gc, "step", gc_step_m, MRB_ARGS_NONE());
}
