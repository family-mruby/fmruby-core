...
/* fmrb: self-supplied timeslice -- BEGIN (doc/wasm/report/p2.md)

   With MRB_TASK_TICK_SELF_SUPPLY the VM produces its own ticks instead of
   waiting for a separate timer task to set task.switching. It has to happen
   here: a CPU-bound Ruby program leaves the scheduler only when switching is
   already true, so the one place that is guaranteed to be reached over and over
   while such a program runs is the bytecode boundary below.

   Every MRB_TASK_SELF_TICK_INTERVAL instructions the HAL is asked to convert
   elapsed wall-clock time into ticks (mrb_hal_task_self_tick, in mruby-task's
   FreeRTOS port). The countdown lives on mrb_vm_exec's own stack, so nothing is
   added to mrb_state and nothing is shared between threads.

   Undefined (the default), this expands to nothing at all and the file is
   byte-for-byte the upstream one. */
#if defined(MRB_TASK_TICK_SELF_SUPPLY)

/* How many bytecodes between calls. It trades VM throughput against how late a
   timeslice boundary is noticed, and the two ends move in opposite directions
   on fast and slow machines:

     - The check itself (one decrement and one branch) is paid per bytecode.
     - The call costs about 780ns on the Linux simulation, which is dominated by
       the FreeRTOS mutex inside the HAL, so its share is 1/N of the run.
     - The delay in noticing a boundary is N bytecodes of VM time: ~27us on the
       Linux simulation (300M bytecodes/s), a few milliseconds on an ESP32.

   8192 keeps the call overhead near 3% where the VM is fast and the detection
   delay well inside a 50ms timeslice where it is slow. Measurements are in
   doc/wasm/report/p2.md. No tick is ever lost by choosing this badly: the HAL
   converts elapsed wall-clock time, so a late call is charged in full. */
#ifndef MRB_TASK_SELF_TICK_INTERVAL
#define MRB_TASK_SELF_TICK_INTERVAL 65536
#endif

/* Declared here rather than through task_hal.h: that header is not on this
   file's include path in the rake mruby build. */
void mrb_hal_task_self_tick(mrb_state *mrb);

/* The countdown starts at 1, not at the interval: the first bytecode after this
   exec begins is where a preemption raised while the task was NOT running gets
   delivered. The scheduler cannot deliver it itself -- execute_task() clears
   mrb->task.switching every time it hands a task the CPU -- so this is the
   moment that stands in for the top-half's asynchronous write. Once it has
   fired, it re-arms with the full interval. */
#define TASK_SELF_TICK_DECL uint32_t task_self_tick_countdown = 1;
#define TASK_SELF_TICK_RESET() (task_self_tick_countdown = 1)
#define TASK_SELF_TICK(mrb) do { \
  if (--task_self_tick_countdown == 0) { \
    task_self_tick_countdown = MRB_TASK_SELF_TICK_INTERVAL; \
    mrb_hal_task_self_tick(mrb); \
  } \
} while (0)

#else

#define TASK_SELF_TICK_DECL
#define TASK_SELF_TICK_RESET() ((void)0)
#define TASK_SELF_TICK(mrb) ((void)0)

#endif
/* fmrb: self-supplied timeslice -- END */
...
/* fmrb: self-supplied timeslice -- no task scheduler, nothing to preempt */
#define TASK_SELF_TICK_DECL
#define TASK_SELF_TICK_RESET() ((void)0)
#endif
...
mrb_vm_exec(mrb_state *mrb, const struct RProc *begin_proc, const mrb_code *iseq)
{
  ...
  TASK_SELF_TICK_DECL  /* fmrb: self-supplied timeslice */
...
RETRY_TRY_BLOCK:
  TASK_SELF_TICK_RESET();  /* fmrb: self-supplied timeslice (the countdown is
                              indeterminate after MRB_CATCH's longjmp) */
