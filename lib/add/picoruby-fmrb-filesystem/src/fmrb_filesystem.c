...
mrb_picoruby_fmrb_filesystem_gem_init(mrb_state *mrb)
{
  ...
  // File class should be defined by mrblib/file.rb before this C code runs
  // Just get the existing class and add methods to it
  if (mrb_class_defined_id(mrb, MRB_SYM(File))) {
    file_class = mrb_class_get_id(mrb, MRB_SYM(File));
  } else {
    // File class doesn't exist (this should not happen with mrblib/file.rb)
    // Create it as a failsafe, inheriting from Object
